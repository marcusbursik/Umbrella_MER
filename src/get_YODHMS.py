...
def get_YODHMS(treal_string, start_filename):
    """
    [Y, D, H, M, S, Y0, D0, H0, M0, S0] = get_YODHMS(treal_string, start_filename)
    ...
    Output:
     ...
     'O' = A vector of the month each image was taken
     ...
     'O0' - The start month of the eruption
    ...
    # Pre-allocare space for the month vector
    O = np.zeros(len(treal_string))
    ...
    for index in range(0,len(treal_string)):
        ...
        Y[index] = float(current_treal[0:4])    # Gets the year 
        O[index] = float(current_treal[4:6])    # Gets the month
        D[index] = float(current_treal[6:8])    # Gets the day
        H[index] = float(current_treal[8:10])    # Gets the hour
        M[index] = float(current_treal[10:12])   # Gets the minute
        S[index] = float(current_treal[12:14])	# Gets the second
	...
	#  format ?YYDDDHHMMSS.
        ...
        Y0 = float(start_time[0:4])     	# Gets the start year
        O0 = float(start_time[4:6])     	# Gets the start month
        D0 = float(start_time[6:8])     	# Gets the start day
        H0 = float(start_time[8:10])     	# Gets the start hour
        M0 = float(start_time[10:12])        # Gets the start minute
        S0 = float(start_time[12:14])       # Gets the start second

    return Y, O, D, H, M, S, Y0, O0, D0, H0, M0, S0
